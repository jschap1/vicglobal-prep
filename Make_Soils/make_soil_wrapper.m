...
% soils = load('/hdd/VICGlobal/07_Archive/v1.6c/Classic/soils_3L_MERIT.txt');
% lon = soils(:,4);
% lat = soils(:,3);
% lonlat = [lon lat];

tz = lonlat(:,1)*24/360;
% tz_mask = xyz2grid(lon, lat, tz);
% figure, plotraster(lon, lat, tz_mask, 'timezone offset')

% Alternative setup using political timezones
